...
static int fetch(const char *url, const char *out_name, int redirects) {
    if (strncmp(url, "http://", 7)) {
        fprintf(stderr, "wget: only http:// URLs are supported (no TLS)\n");
        return 1;
    }
    ...
    if (fd < 0 || connect_ip(fd, ip, port, 10000) < 0) { printf("failed: %s\n", strerror(errno)); return 1; }
    printf("connected.\n");
    char req[800];
    ...
    while (hlen < sizeof(hdr) - 1) {
        long r = read(fd, hdr + hlen, sizeof(hdr) - 1 - hlen);
        if (r <= 0) break;
        hlen += r;
        hdr[hlen] = 0;
        if ((body = strstr(hdr, "\r\n\r\n"))) break;
    ...
    *body = 0;
    ...
    if (out < 0) { printf("cannot create %s: %s\n", name, strerror(errno)); close(fd); return 1; }
    long got = 0;
